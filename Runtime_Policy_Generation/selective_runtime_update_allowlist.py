...
def out_dict_add(outDict, outList):
    ...
    outDict[str(pkgName[1] + "__" + version)] = newDict
    return outDict
...
def write_allowlist(hashDict):
    ...
    with open("/tmp/allowlist_config/universe_allowlist.txt", "a+") as f:
        for i in hashDict:
...
def main():
    ...
    masterUniPath = '/tmp/allowlist_config/MasterUniverse.txt'

    # returns a dictionary
    masterDict = clean_package_file(masterUniPath)
    os.chdir('/tmp/allowlist_config/')
    ...
    if args.generate:
        ...
        print()
        print(">>> Measuring the Master Repository....\n")
        create_allowlist_update(masterDict, args.exec)
        os.chdir("/tmp/allowlist_config/")
